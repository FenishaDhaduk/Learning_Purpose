...
const ChatWindow = ({ selectedUser }) => {
  ...
  const handleSendMessage = () => {
    if (inputMessage.trim()) {
      socket.emit('sendMessage', { senderId: user?.id, receiverId: selectedUser._id, content: inputMessage });
      setInputMessage('');
      setIsTyping(false);
      socket.emit('stopTyping', { senderId: user?.id, receiverId: selectedUser._id });
    }
  };
 ...
 const handleInputChange = (e) => {
  ...
  const renderMessagesWithHeaders = () => {
    ...
    messages.forEach((message, index) => {
      ...
      const shouldShowDateHeader = !previousMessage || !isSameDay(messageDate, previousDate);
  ...
  const handleScroll = () => {
    ...
    const visibleHeaders = dateHeaders.filter((header) => header.offsetTop <= scrollPosition + 20);
  ...
  return (
    <div className="chat-window">
      ...
      <div className="sticky-date-header">{stickyHeader}</div> 
